...
| 难度 | 类型 | 类型说明 |
|---|---|---|
| 易 | `DUPLICATE_DETAIL_IMAGE` | 两张细节图完全相同，或经过轻微缩放、裁剪、压缩等处理后仍属于重复图。 |
| 易 | `IMAGE_QUALITY` | 主图或任意一张细节图存在模糊、遮挡或分辨率过低等质量问题。 |
| 易 | `PASS` | 商品标题、品类、颜色、材质、主图和两张细节图之间不存在目标错误。 |
| 中 | `CATEGORY_MISMATCH` | `category` 字段所表示的商品品类与标题及商品图片中的实际品类不一致。 |
| 中 | `WRONG_DETAIL_IMAGE` | 主图和文本信息正确，但两张细节图中有一张不属于当前商品。 |
| 中 | `COLOR_MISMATCH` | `color` 字段非空，但其颜色值与商品图片中可验证的商品颜色不一致。 |
| 难 | `TITLE_MISMATCH` | `title` 描述的商品身份、型号、用途或关键内容与主图及细节图中的商品不一致。 |
| 难 | `MATERIAL_MISMATCH` | `material` 字段非空，但其材质值与图片中能够可靠验证的商品材质不一致。 |
