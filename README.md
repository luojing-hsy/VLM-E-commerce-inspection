...
SFT 的类别配额为：训练集易/中类各 150、难类各 50；验证集易/中类各 15、难类各 5。SFT 每个商品保留 0–3 条，当前训练集最多 3 条、验证集最多 1 条。Joint 已从 `data/joint_synthesis` 重新抽样，训练集按易类 4×75、中类 2×200、难类 2×150 配额，验证集按易类 8/8/7/7、中类 2×20、难类 2×15 配额；每个来源商品只保留 1 条。
...
每个当前数据集都带有自己的 `images/` 目录。JSONL 中的 `images.main.image_id` 和 `images.detail[*].image_id` 指向对应数据集目录内的实际文件；SFT 路径统一为 `data/sft/images/...`，Joint 路径统一为 `data/joint/images/...`。Joint 重构会重新生成 `IMAGE_QUALITY` 退化图，不复用旧的子类型—位置绑定。

## 目录说明

- 当前数据：`data/sft`、`data/joint`、`data/test`。
- 源数据与审计：`data/all_product.jsonl`、`data/raw_clean`、`data/manifests`。
- `data/joint_synthesis` 是 Joint 重构的候选池；`data/sft_synthesis`、`data/prepared`、`data/highres_split` 是其他中间产物，不作为最终规范数据集。
- SFT/Joint 入口、评估配置和基线脚本已切换到当前数据目录；启动前会在 outputs 下生成 veRL 运行时输入。

## Qwen3.5 runtime

The active training path targets Qwen3.5-4B. Model loading uses Transformers AutoConfig/AutoProcessor/AutoModelForImageTextToText and validates the model type before SFT, validation, test, joint OPD, or GRPO.
...
- `bash scripts/joint.sh --dry-run` checks the joint GRPO + OPD command; `bash scripts/joint.sh` launches it.
- `bash scripts/grpo_on_joint.sh --dry-run` checks the GRPO-on-joint command; `bash scripts/grpo_on_joint.sh` launches it.
